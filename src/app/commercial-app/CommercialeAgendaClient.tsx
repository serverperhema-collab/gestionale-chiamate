...
export default function CommercialeAgendaClient() {
  ...
  const fetchAppointments = async () => {
    try {
      setLoading(true);
      // Faccio una fetch all'endpoint che uso anche nel TL, passando magari un parametro, 
      // ma il backend del commerciale restituisce gia i suoi
  ...
  // 1. Appuntamenti da Svolgere (Futuri o oggi, NON ESITATI)
  ...
  // 2. Appuntamenti Svolti Esitati (Status DONE o hanno un outcome)
  const svoltiEsitati = appointments.filter(a => {
    if (a.status === "DA_GESTIRE_COMMERCIALE") return false;
    return a.status === "DONE" || (a.outcomes && a.outcomes.length > 0);
  });

  // 3. Appuntamenti Svolti da Esitare (Passati, NON ESITATI)
  ...
  // 4. Richiami Personali
  // Sono gli appuntamenti fittizi creati per i richiami al commerciale
  const richiamiPersonali = appointments.filter(a => a.status === "DA_GESTIRE_COMMERCIALE");

  // Helper per analizzare gli esiti
  const getLatestOutcome = (a: any) => {
    if (!a.outcomes || a.outcomes.length === 0) return null;
    return a.outcomes[0]; // Assumendo che il primo sia il piu recente grazie a orderBy nel backend
  };

  // 5. Richiami Assegnati ad Operatori
  ...
  return (
    <div className="p-4 max-w-[1400px] mx-auto pb-12 flex flex-col md:flex-row gap-6 h-[calc(100vh-4rem)]">
          ...
          <button
            onClick={() => {
              if (displayedAppts.length > 0) {
                ...
                toast.error("Nessun appuntamento da stampare");
              }
            }}
                  ...
                  <div className={`px-4 py-3 border-b flex justify-between items-center ${isDaGestire ? 'bg-indigo-900/20 border-indigo-800/30' : isUnconfirmed ? 'bg-yellow-900/20 border-yellow-800/30' : 'bg-gray-800 border-gray-700'}`}>
                    <div className="flex flex-col">
                      ...
                      <div className="flex gap-2 mt-1">
                        {appt.isPhoneAppt && (
                          <span className="text-[10px] font-bold uppercase tracking-wider bg-purple-900/50 text-purple-300 px-2 py-0.5 rounded border border-purple-500/50 inline-flex items-center">
                            <PhoneCall className="w-3 h-3 mr-1" /> Telefonico
                          </span>
                        )}
                        {appt.isSecondAppt && (
                          <span className="text-[10px] font-bold uppercase tracking-wider bg-teal-900/50 text-teal-300 px-2 py-0.5 rounded border border-teal-500/50 inline-flex items-center">
                            <RefreshCw className="w-3 h-3 mr-1" /> 2° App
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {o && o.outcomeFinal === "STANDBY" && (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-orange-900/50 text-orange-400 px-2 py-0.5 rounded border border-orange-700/50 flex items-center">
                          <PauseCircle className="w-3 h-3 mr-1" /> Standby
                        </span>
                      )}
                      {o && o.outcomeFinal === "VENDUTO" && (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-emerald-900/50 text-emerald-400 px-2 py-0.5 rounded border border-emerald-700/50 flex items-center">
                          <Handshake className="w-3 h-3 mr-1" /> Venduto
                        </span>
                      )}
                      {o && o.outcomeFinal === "KO" && (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-red-900/50 text-red-400 px-2 py-0.5 rounded border border-red-700/50 flex items-center">
                          <XCircle className="w-3 h-3 mr-1" /> KO Definitivo
                        </span>
                      )}
                      {isDaGestire ? (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-indigo-900/50 text-indigo-400 px-2 py-0.5 rounded border border-indigo-700/50">
                          Richiamo (Da Gestire)
                        </span>
                      ) : isUnconfirmed ? (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-yellow-900/50 text-yellow-400 px-2 py-0.5 rounded border border-yellow-700/50 flex items-center">
                          <AlertTriangle className="w-3 h-3 mr-1" /> Da Confermare
                        </span>
                      ) : (
                  ...
                  <div className="p-4 flex flex-col md:flex-row gap-4 justify-between">
                    <div className="space-y-3 flex-1">
                      ...
                      <div className="bg-gray-950 p-3 rounded-xl border border-gray-800 space-y-2">
                        <div className="flex items-center text-sm">
                          ...
                          <span className="text-gray-300"><strong className="text-white">{appt.referentName}</strong> - {appt.referentRole}</span>
                        </div>
                        <div className="flex items-center text-sm">
                          <Phone className="w-4 h-4 text-gray-500 mr-2 shrink-0" />
                          <span className="text-blue-400 font-mono">{appt.phone}</span>
                        </div>
                        <div className="flex items-start text-sm">
                          <FileText className="w-4 h-4 text-gray-500 mr-2 shrink-0 mt-0.5" />
                          <span className="text-gray-300 italic">"{appt.clientNeeds || "Nessuna nota iniziale"}"</span>
                        </div>
                        {o && (
                          <div className="mt-2 pt-2 border-t border-gray-800 text-sm flex items-start">
                             ...
                             <span className="text-gray-300">Esito registrato: <strong className="text-white">{o.notes}</strong></span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 min-w-[140px] justify-center">
                      <button 
                        ...
                        className="w-full px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-xl transition shadow-lg shadow-blue-900/20"
                      >
...
function TabButton({ id, label, count, active, setActive, color }: { id: TabType, label: string, count: number, active: TabType, setActive: (t: TabType) => void, color: string }) {
  ...
  return (
    ...
    >
      ...
      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isActive ? 'bg-black/20 text-white' : 'bg-gray-700 text-gray-300'}`}>
        {count}
      </span>
    </button>
  );
