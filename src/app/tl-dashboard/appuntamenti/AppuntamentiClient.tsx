...
export default function AppuntamentiClient() {
  const [activeTab, setActiveTab] = useState<"IN_GESTIONE" | "FIRMATE" | "KO">("IN_GESTIONE");
    ...
    <main className="flex-1 p-4 md:p-8 overflow-y-auto">
      <div className="max-w-[1600px] mx-auto h-[85vh] flex flex-col">
        <div className="flex bg-gray-900 border border-gray-700 rounded-lg p-1 mb-6 shrink-0 w-fit">
          ...
          <button 
            onClick={() => setActiveTab("FIRMATE")}
            className={`px-6 py-2.5 text-sm font-bold rounded-md transition ${activeTab === "FIRMATE" ? "bg-green-600 text-white shadow-md" : "text-gray-400 hover:text-gray-200"}`}
          >
            CONTRATTO FIRMATO
          </button>
          <button 
        ...
        <div className="flex-1 overflow-hidden">
          <TrattativeTable 
            ...
            title={activeTab === "IN_GESTIONE" ? "Appuntamenti in Gestione" : activeTab === "FIRMATE" ? "Contratti Firmati" : "Appuntamenti Persi (KO)"}
          />
        </div>
      </div>
    </main>
