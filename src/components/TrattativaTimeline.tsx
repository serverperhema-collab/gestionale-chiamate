...
export default function TrattativaTimeline({ trattativaId, onClose }: TrattativaTimelineProps) {
  ...
  const EditableField = ({ label, field, value }: { label: string, field: string, value: string }) => {
    ...
    return (
      <div className="flex flex-col bg-gray-800/50 p-3 rounded-lg border border-gray-700/50">
        ...
        {isEditing ? (
          <div className="flex items-center">
            <input 
              ...
              autoFocus
            />
            <button onClick={() => handleSaveField(field)} disabled={savingEdit} className="ml-2 p-1.5 bg-green-600 hover:bg-green-500 rounded text-white transition">
              {savingEdit ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
            </button>
            <button onClick={() => setEditingField(null)} className="ml-1 p-1.5 bg-gray-700 hover:bg-gray-600 rounded text-white transition">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
        ) : (
          <div className="flex items-center justify-between group cursor-pointer" onClick={() => setEditingField(field)}>
            ...
            <Edit2 className="w-3 h-3 text-gray-500 opacity-0 group-hover:opacity-100 transition" />
          </div>
        )}
      </div>
    );
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8">
      ...
      <div className="relative w-full h-full max-w-6xl bg-gray-900 border border-gray-700 shadow-2xl rounded-2xl flex flex-col overflow-hidden">
        ...
        <div className="bg-gray-800 p-6 border-b border-gray-700 shrink-0">
          <h1 className="text-3xl md:text-4xl font-black text-white text-center uppercase tracking-tight mb-6">
            {contact.name}
          </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
            ...
            <EditableField label="Indirizzo" field="address" value={trattativa.contact?.address} />
            <EditableField label="Settore" field="sector" value={trattativa.contact?.sector} />
          </div>
        </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-6">
              {trattativa.history && trattativa.history.length > 0 ? (
                // L'ordinamento dal database di solito è desc (più recenti prima), li rovesciamo se vogliamo vedere la storia dall'inizio, oppure teniamo dal più recente in alto.
                // Il testo dice "Il primo sarà sempre da chi è stata creata...", quindi ordine cronologico (ascendente)
                    ...
                    <div key={h.id} className="relative flex items-start">
                      ...
                      <div className="ml-6 flex-1 bg-gray-800/80 border border-gray-700 rounded-xl p-5 shadow-sm">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="font-bold text-white text-lg uppercase">{h.actionType.replace(/_/g, " ")}</h4>
                        ...
                        <p className="text-sm text-gray-300 italic whitespace-pre-wrap leading-relaxed">
                          {h.notes || "Nessuna nota aggiuntiva."}
                        </p>
